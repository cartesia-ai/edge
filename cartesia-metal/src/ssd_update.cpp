...
void SSDUpdate::eval_gpu(const std::vector<array>& inputs, std::vector<array>& outputs) {
  ...
  compute_encoder->setBytes(&group_size, sizeof(size_t), 10);
  compute_encoder->setBytes(&state_size, sizeof(size_t), 11);

  compute_encoder->setBytes(x.strides().data(), 3 * sizeof(size_t), 12);
  compute_encoder->setBytes(dt.strides().data(), 2 * sizeof(size_t), 13);
  compute_encoder->setBytes(B.strides().data(), 3 * sizeof(size_t), 14);
  compute_encoder->setBytes(state.strides().data(), 4 * sizeof(size_t), 15);
