...
void SSDUpdateNoZ::eval_gpu(const std::vector<array>& inputs, std::vector<array>& outputs) {
  ...
  compute_encoder.set_bytes(&group_size, sizeof(size_t), 9);
  compute_encoder.set_bytes(&state_size, sizeof(size_t), 10);

  compute_encoder.set_bytes(x.strides().data(), 3 * sizeof(size_t), 11);
  compute_encoder.set_bytes(dt.strides().data(), 2 * sizeof(size_t), 12);
  compute_encoder.set_bytes(B.strides().data(), 3 * sizeof(size_t), 13);
  compute_encoder.set_bytes(state.strides().data(), 4 * sizeof(size_t), 14);
